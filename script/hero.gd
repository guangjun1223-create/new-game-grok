...
var _current_attack_animation_name: String = "AttackSword"
...
func _update_weapon_visuals():
	# Kiểm tra an toàn
	if weapon_container == null: return

	# BƯỚC 1: TẮT TẤT CẢ CÁC SPRITE VŨ KHÍ TRƯỚC
	...
	# BƯỚC 2: NẾU CÓ VŨ KHÍ, BẬT SPRITE TƯƠNG ỨNG VÀ CHỌN ANIMATION
	if typeof(weapon_id) == TYPE_STRING and not weapon_id.is_empty():
		...
		# Quyết định sprite nào sẽ hiện và animation nào sẽ dùng
		match weapon_type:
			"BOW":
				target_sprite_name = "BowSprite"
				_current_attack_animation_name = "AttackBow"
			"SWORD":
				target_sprite_name = "SwordSprite"
				_current_attack_animation_name = "AttackSword"
			# Mặc định cho các loại vũ khí khác (ví dụ: DAGGER)
			_: 
				target_sprite_name = "SwordSprite"
				_current_attack_animation_name = "AttackSword"
		
		# === LỆNH PRINT DEBUG BẠN YÊU CẦU ===

		# ======================================

		# Tìm và bật sprite tương ứng
	...
	# BƯỚC 3: NẾU KHÔNG CÓ VŨ KHÍ, RESET ANIMATION
	else:
		_current_attack_animation_name = "AttackSword"
	...
	match _current_state:
		State.IDLE:
			...
			if is_instance_valid(state_timer):
				state_timer.start(randf_range(idle_duration - 0.5, idle_duration + 0.5))
		...
		State.WANDER:
			if cap_nhat_ranh_gioi_di_chuyen():
				...
				# === KIỂM TRA AN TOÀN ===
				if is_instance_valid(state_timer): state_timer.start(randf_range(1.0, 2.0))
			else:
		...
		State.COMBAT:
			...
			# Kiểm tra an toàn cho SkillTimer trước khi sử dụng
			if is_instance_valid(skill_timer):
	...
	if velocity.length() < 1.0:
		...
		match weapon_type:
			...
			_: # Kiếm, dao găm và các loại khác
				...
				# Sát thương cận chiến sẽ được xử lý bằng Method Track trong animation
...
func _on_animation_finished(anim_name): 
	...
	if anim_name in ["AttackSword", "AttackBow"]:
		_is_attacking = false
		has_dealt_damage_this_attack = false
		animation_player.play(&"Idle")
...
func _xu_ly_tu_dong_dung_sp_potion():
	if is_dead or max_sp <= 0: return # Không xử lý nếu đã chết hoặc không có SP

	# Điều kiện SP: Dưới 30% thì bắt đầu tìm bình SP
	...
	# Nhờ UI tìm giúp một ô Potion sẵn sàng (hàm này ta sẽ sửa ở UI)
...
func dung_potion(slot_key: String):
	...
	# === PHẦN NÂNG CẤP ===
	# Kiểm tra xem bình này hồi HP hay SP
	...
	if sp_restore_amount > 0:
		...
		sp_changed.emit(current_sp, max_sp) # Phát tín hiệu để UI cập nhật
		FloatingTextManager.show_text("+" + str(sp_restore_amount), Color.BLUE, text_position)

	# Phần logic trừ item và cooldown giữ nguyên
...
func nang_cap_chi_so(stat_name: String):
	...
	# 4. Cập nhật lại tất cả chỉ số phụ (HP, ATK, DEF...)
	_update_secondary_stats()
...
func _activate_skill(skill_id: String):
	...
	match skill_id:
		"NOV_SWORD_BOOM":
			...
			attack_timer.stop()
			animation_player.play("AttackSword") # Dùng tạm animation đánh thường, sau này có thể đổi
...
func _spawn_vfx(animation_name: String, offset_y: float = -60.0):
